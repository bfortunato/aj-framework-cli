...
function imagesSvg(platforms) {
    var images = [];
    gulp.src(["app/resources/images/**/*.svg"]).pipe(through2.obj(function (chunk, enc, cb) {
        console.log("Working on " + chunk.path);
        if (chunk.path.indexOf("@") == -1) {
            //already converted
            easyimage.info(chunk.path).then(function (info) {
                ...
                for (var i = 0; i < ratios.length; i++) {
                    (function (m) {
                        var factor = m;
                        pn.readFile(chunk.path).then(function (buffer) {
...
function imagesRaster(platforms) {
    var images = [];
    gulp.src(["app/resources/images/**/*.png", "app/resources/images/**/*.jpg"]).pipe(through2.obj(function (chunk, enc, cb) {
        if (chunk.path.indexOf("@") == -1) {
            //already converted
            easyimage.info(chunk.path).then(function (info) {
                var ext = path.extname(chunk.path).toLowerCase();
                        ...
                        (function (m) {
                            ...
                            if (platform.ratios.indexOf(m) != -1) {
                                ...
                                easyimage.resize({
                                    ...
                                    console.log("Error resizing image " + chunk.path + " with factor " + factor + "(w=" + info.width * factor + ", h=" + info.height * factor + ")" + err);
                                });
                            }
                        })(ratios[i]);
...
function scripts(platform) {
    try {
        gulp.src("app/js/**/*.js").pipe(babel({ presets: "es2015" }).on("error", function (e) {
            console.log(e.stack);
